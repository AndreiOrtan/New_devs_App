...
// Rounds a decimal string to 2 places using integer (BigInt) arithmetic so
// currency totals never round-trip through binary floating point.
function formatCurrency(decimalStr: string): string {
    const trimmed = decimalStr.trim();
    const negative = trimmed.startsWith('-');
    const abs = negative ? trimmed.slice(1) : trimmed;
    const [intPart, fracPart = ''] = abs.split('.');
    const threeDp = (fracPart + '000').slice(0, 3);

    const thousandthsOfADollar = BigInt(intPart || '0') * 1000n + BigInt(threeDp);
    const roundedCents = (thousandthsOfADollar + 5n) / 10n; // round-half-up to cents
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    const displayTotal = formatCurrency(data.total_revenue);
